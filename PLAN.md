...
| **A6 Transmit plugin** ✅ | The real device: argb32f/bgra32f in working space (a fresh `PrSDKString` per mode), flipped, reordered and converted to `RGBA16F` in one pass (D1, D5). Ships Premiere support (D6) | Flip + reorder + half pass benchmarked in `qcbae-convbench` at 4K; appears in Preferences → Video Preview in both apps; `qcbae-probe dump` matches the AEGP tap on the same comp. **Done 2026-09-21** — 2.31 ms at 4K (ARGB, bottom-up), never clamps, inf/NaN flagged; `lab/results/2026-09-21-a6-transmit-device/` |
| **A3 QCView ingest** ✅ | Ring-reader live source in QCView (GPL, that repo), the 16F upload branch in both renderers, routing split from SRT. Single view only | Comp is live in QCView as a media item and follows AE; OCIO engaged by hand gives the expected picture; dropouts hold the last frame; bytes match `qcbae-probe dump` **Done 2026-09-21** (QCView branch `qcbae-live`, unpushed): bit-exact against the synthetic producer, media matrix clean, AE + Premiere measured 2026-09-22: values equal the ring, inf flagged and badged, Paused hint, 4K at 24 fps (copy 2.8 ms). `lab/results/2026-09-21-a3-qcview-ingest/`. Merged to QCView `main` 2026-09-22, together with work A3 deliberately left out: **live sources in dual view** (either side, or both), the threading fixes that made that safe, and a network-volume read-ahead. |
...
| **A7 Packaging** | Signing, notarization, installers both platforms | Installs clean on a machine that has never seen the SDK. *QCView's half is done (2026-09-22): its macOS release flow was rebuilt and proven end to end — signed, notarized, stapled, `scripts/RELEASE.md` in that repo. QCBridgeAE's own half is untouched: the Transmit bundle is still copied into MediaCore by hand, unsigned.* |
