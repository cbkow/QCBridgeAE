...
contract between the two machines. A session starts with
`WINDOWS-SESSION.md`; the sequence across all three repos (QCView,
QCBridgeAE, QCBridge) is `PLAN-windows.md`; the checklist of open Windows
work is `TRACKING-windows.md`.

## Before anything builds

**Get your own copy of the Adobe SDKs.** They are not in this repo and never
will be — the headers carry an ADOBE CONFIDENTIAL notice (`DESIGN-NOTES.md` §Privacy).
...
Phase **A5** (`DESIGN-NOTES.md`): Windows parity for the ring, the Transmit device
(the product since 2026-09-21, `DESIGN-NOTES.md` D7) and, as a test instrument, the
AEGP tap.

The macOS side does **not** use IOSurface any more — it is a page-aligned
POSIX shared mapping (`DESIGN-NOTES.md` D8, measured zero-copy on Apple Silicon). The
...
the plugin (`DESIGN-NOTES.md` D1, Transmit note), so on Windows this is the main path,
...
If you hit something that contradicts `DESIGN-NOTES.md`, say so in your notes and flag
